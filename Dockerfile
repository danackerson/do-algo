...
ENTRYPOINT ansible-playbook deploy.yml -t digitalocean,vpn,cloud \
  ...
  && for filepath in `find configs/ -type f -regextype posix-extended -regex '.*/[0-9{1,}.[0-9]{1,}.[0-9]{1,}.[0-9]{1,}/.*'`;do filename=`basename $filepath`;curl -F "fileupload=@$filepath" "https://ackerson.de/up/$filename?tok=$FILE_TOKEN";done \
