...
ENTRYPOINT ansible-playbook deploy.yml -t digitalocean,vpn,cloud \
  ...
  && for filepath in `find /algo-master/configs -name '*' -type f`;do filename=`basename $filepath`;curl -F "fileupload=@$filepath" "https://ackerson.de/up/$filename?tok=$FILE_TOKEN";done \
